...
export class ServerAuth {
    ...
    logger;
    constructor(key, validHostnames, opts) {
        this.key = key;
    ...
    requestListener(httpAuthedHandler) {
        return (req, res) => {
            Promise.resolve()
                .then(async () => {
    ...
    async authenticateRequest(hostname, authHeader) {
    ...
    async returnChallenge(hostname, clientPublicKey, returnParams) {
    ...
    genBearerToken(clientPeerId, hostname) {
    ...
    async unwrapBearerToken(expectedHostname, token) {
        if (token.length < PeerIDAuthScheme.length + 1) {
            throw new Error('Invalid bearer token');
        }
        const bearer = parseHeader(token).bearer;
        const unwrapped = await this.verifyBox(this.key.publicKey, bearer);
    ...
    async unwrapOpaque(opaque) {
        const unwrapped = await this.verifyBox(this.key.publicKey, opaque);
