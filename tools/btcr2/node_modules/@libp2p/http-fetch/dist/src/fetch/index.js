 ...
 */
async function detectBrokenRequestBody() {
 ...
 */
export function fetchViaDuplex(s) {
    return async (req) => {
 ...
 */
export async function handleRequestViaDuplex(s, h) {
 ...
 */
export function readHTTPMsg(expectRequest, r) {
    const msgPromise = defer();
    return [
        msgPromise.promise,
        (async () => {
...
async function writeRequestToDuplex(s, request) {
    ...
    if (reqUrl.startsWith(multiaddrURIPrefix)) {
        ...
        const [, httpPathVal] = ma.stringTuples().find(([code, value]) => code === protocols('http-path').code) ?? ['', ''];
...
async function writeResponseToDuplex(s, resp) {
