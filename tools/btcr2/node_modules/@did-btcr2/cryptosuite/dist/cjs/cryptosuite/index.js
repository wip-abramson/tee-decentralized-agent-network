import { Canonicalization, CryptosuiteError, DateUtils, MethodError, PROOF_GENERATION_ERROR, PROOF_SERIALIZATION_ERROR, PROOF_VERIFICATION_ERROR } from '@did-btcr2/common';
 ...
 */
export class BIP340Cryptosuite {
    /**
    ...
    transformDocument(document, config) {
        ...
        if (config.type !== this.type) {
            throw new MethodError('Type mismatch: config.type !== this.type', PROOF_VERIFICATION_ERROR, { config, this: this });
        }
        // Get the cryptosuite from the options and if it matches this cryptosuite
        if (config.cryptosuite !== this.cryptosuite) {
            throw new MethodError('Cryptosuite mismatch: config.cryptosuite !== this.cryptosuite', PROOF_VERIFICATION_ERROR, { config, this: this });
        }
     ...
     */
    proofConfiguration(config) {
        // If the config type does not match the cryptosuite type, throw CryptosuiteError
        if (config.type !== this.type) {
            throw new CryptosuiteError('Type mismatch: config.type !== this.type', PROOF_GENERATION_ERROR, { config, this: this });
        }
        // If the cryptosuite does not match the cryptosuite name, throw CryptosuiteError
        if (config.cryptosuite !== this.cryptosuite) {
            throw new CryptosuiteError('Cryptosuite mismatch: config.cryptosuite !== this.cryptosuite', PROOF_GENERATION_ERROR, { config, this: this });
        }
        ...
        if (config.created) {
            ...
            if (!DateUtils.isValidXsdDateTime(config.created))
                throw new CryptosuiteError('Invalid config: "created" must be a valid XMLSchema DateTime string', PROOF_GENERATION_ERROR, config);
        }
     ...
     */
    proofSerialization(hash, config) {
        ...
        if (config.verificationMethod !== this.multikey.fullId()) {
            ...
            throw new CryptosuiteError('Id mismatch: config.verificationMethod !== this.multikey.fullId()', PROOF_SERIALIZATION_ERROR, { config, this: this });
        }
     ...
     * @throws {CryptosuiteError} if the multikey does not match the verification method.
     */
    proofVerification(hash, signature, config) {
        // If the config verification method !== the multikey fullId, throw CryptosuiteError
        if (config.verificationMethod !== this.multikey.fullId()) {
            throw new CryptosuiteError(`Id mismatch: config.verificationMethod !== this.multikey.fullId()`, PROOF_VERIFICATION_ERROR, { config, this: this });
        }
