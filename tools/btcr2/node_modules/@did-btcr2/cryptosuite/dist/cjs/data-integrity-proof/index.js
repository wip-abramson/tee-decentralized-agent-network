 ...
 */
export class BIP340DataIntegrityProof {
    ...
    addProof(unsignedDocument, config) {
        ...
        if (!proof.type || !proof.verificationMethod || !proof.proofPurpose) {
            throw new DataIntegrityProofError('Invalid proof: missing proof.type, proof.verificationMethod and/or proof.proofPurpose', PROOF_GENERATION_ERROR, { config, proof });
        }
        ...
        if (config.domain) {
            ...
            if (proof.domain !== config.domain)
                throw new DataIntegrityProofError('Domain mismatch: proof.domain !== config.domain', PROOF_GENERATION_ERROR, { config, proof });
        }
        // Check if the config has a challenge
        if (config.challenge) {
            // Check that it matches the proof.challenge
            if (proof.challenge !== config.challenge)
                throw new DataIntegrityProofError('Challenge mismatch options and challenge passed', PROOF_GENERATION_ERROR, { config, proof });
        }
        // Cast the unsignedDocument to a SignedBTCR2Update to add the proof
        const signedDocument = unsignedDocument;
     ...
     */
    verifyProof(document, expectedPurpose, mediaType, expectedDomain, expectedChallenge) {
        try {
            // Parse the document
            const signedDocument = JSON.parse(document);
            ...
            if (!proof.type || !proof.verificationMethod || !proof.proofPurpose) {
                throw new DataIntegrityProofError('Invalid proof: missing proof.type, proof.verificationMethod and/or proof.proofPurpose', PROOF_VERIFICATION_ERROR, signedDocument);
            }
            ...
            if (expectedPurpose)
                ...
                if (expectedPurpose !== proof.proofPurpose)
                    ...
                    throw new DataIntegrityProofError('Proof purpose mismatch: proof.proofPurpose !== expectedPurpose', PROOF_VERIFICATION_ERROR, { proof, expectedPurpose });
            ...
            if (expectedChallenge)
                ...
                if (expectedChallenge !== proof.challenge)
                    // Else throw DataIntegrityProofError
                    throw new DataIntegrityProofError('Challenge mismatch: proof.challenge !== expectedChallenge', 'INVALID_CHALLENGE_ERROR', { proof, expectedChallenge, });
            ...
            if (expectedDomain) {
                ...
                if (Array.isArray(expectedDomain) && expectedDomain.length) {
                    ...
                    if (expectedDomain.length !== proof.domain?.length) {
                        // Else throw DataIntegrityProofError
                        throw new DataIntegrityProofError('Domain mismatch: expectedDomain length does not match proof.domain length', PROOF_VERIFICATION_ERROR, { proof, expectedDomain });
                    }
                    // Check that each entry in expectedDomain can be found in proof.domain
                        ...
                        // Else throw DataIntegrityProofError
                        throw new DataIntegrityProofError('Domain mismatch: expectedDomain and proof.domain do not match', PROOF_VERIFICATION_ERROR, { proof, expectedDomain });
                    }
                }
                // Else expectedDomain is a string, check that it matches proof.domain
                else if (proof.domain !== expectedDomain) {
                    throw new DataIntegrityProofError('Domain mismatch: proof.domain !== expectedDomain', PROOF_VERIFICATION_ERROR, { proof, expectedDomain });
                }
            }
            // Verify the proof
            ...
            throw new DataIntegrityProofError('Error verifying proof: ' + (error instanceof Error ? error.message : String(error)), PROOF_VERIFICATION_ERROR, { document, mediaType, expectedPurpose, expectedDomain, expectedChallenge });
        }
    }
}
